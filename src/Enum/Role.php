<?php

declare(strict_types=1);

namespace App\Enum;

enum Role: string
{
    case Neutral = 'neutral';
    case Wife = 'wife';
    case Sister = 'sister';
    case Teammate = 'teammate';

    public function label(): string
    {
        return match ($this) {
            self::Neutral => '🌟 Нейтральная',
            self::Wife => '💝 Жена',
            self::Sister => '✨ Сестра',
            self::Teammate => '🤝 Тиммейт',
        };
    }

    public function emoji(): string
    {
        return match ($this) {
            self::Neutral => '🌟',
            self::Wife => '💝',
            self::Sister => '✨',
            self::Teammate => '🤝',
        };
    }

    public function buildPrompt(?string $name, array $previousCompliments = []): string
    {
        $namePhrase = $name ? " для {$name}" : '';

        $prompt = match ($this) {
            self::Neutral => <<<PROMPT
                Напиши одно тёплое, приятное сообщение{$namePhrase} на русском языке.

                Требования:
                - Универсальный тон: подходит для любого человека
                - Позитивное, подбадривающее настроение
                - Можно использовать 1-2 эмодзи (но не обязательно)
                - Не более 2-3 предложений
                - Без кавычек и префиксов
                - Просто текст сообщения

                Напиши только сообщение, без дополнительных пояснений.
                PROMPT,

            self::Wife => <<<PROMPT
                Напиши один красивый, искренний и романтичный комплимент{$namePhrase} на русском языке.

                Требования:
                - Комплимент должен быть тёплым и нежным
                - Не более 2-3 предложений
                - Без кавычек и префиксов типа "Комплимент:"
                - Просто текст комплимента

                Напиши только комплимент, без дополнительных пояснений.
                PROMPT,

            self::Sister => <<<PROMPT
                Напиши одно тёплое подбадривающее сообщение{$namePhrase} — для 10-летней девочки-школьницы на русском языке.

                Требования:
                - Дружеское и позитивное настроение
                - Мотивация, поддержка в учёбе или добрые слова
                - Можно использовать 1-2 эмодзи (но не обязательно)
                - Не более 2 предложений
                - Без кавычек и префиксов
                - Просто текст сообщения

                Напиши только сообщение, без дополнительных пояснений.
                PROMPT,

            self::Teammate => <<<PROMPT
                Напиши одно тёплое, мотивирующее напутствие{$namePhrase} от тимлида на русском языке.

                Требования:
                - Тон: дружелюбный, поддерживающий, как от старшего коллеги
                - Подходит одинаково для разработчиков, тестировщиков и продуктов
                - Мотивация, поддержка, признание вклада в команду
                - Можно использовать 1-2 эмодзи (но не обязательно)
                - Не более 2-3 предложений
                - Без кавычек и префиксов
                - Просто текст сообщения

                Напиши только сообщение, без дополнительных пояснений.
                PROMPT,
        };

        if (!empty($previousCompliments)) {
            $list = '';
            foreach ($previousCompliments as $i => $text) {
                $list .= ($i + 1) . '. ' . $text . "\n";
            }

            $prompt .= <<<DEDUP

                ВАЖНО: Ниже приведены ранее отправленные сообщения. Твоё новое сообщение должно быть уникальным
                и НЕ повторять ни одно из них по смыслу, структуре или ключевым фразам:
                {$list}
                DEDUP;
        }

        return $prompt;
    }
}
